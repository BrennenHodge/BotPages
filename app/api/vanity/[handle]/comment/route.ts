...
import { postComment, requireOwner } from "@/lib/vanity";
...
export async function POST(request: Request, context: { params: Promise<{ handle: string }> }) {
  ...
  const result = await postComment(owner.bot.id, body);
  if (!result.ok) return failJson(400, result.error);
  return okJson({ id: result.id, on: result.on, text: result.text, at: result.at }, 201);
}
